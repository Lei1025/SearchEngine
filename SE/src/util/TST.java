...
public class TST<Value> {
	...
	// all keys starting with given prefix by user input
	...
	public Iterable<String> prefixMatchLm(String prefix, int count) {
		Queue<String> queue = new Queue<String>();
		Node x = get(root, prefix, 0);
		if (x == null)
			return queue;
		if (x.val != null)
			queue.enqueue(prefix);
		collect(x.mid, prefix, queue, count);
		return queue;
	}
	...
	private void collect(Node x, String prefix, Queue<String> queue, int count) {
		if (queue.size() >= count) {
			return;
		}
		if (x == null)
			return;
		collect(x.left, prefix, queue, count);
		if (x.val != null)
			queue.enqueue(prefix + x.c);
		collect(x.mid, prefix + x.c, queue, count);
		collect(x.right, prefix, queue, count);
	}
